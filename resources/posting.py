...
class PostingResource(Resource) :
    ...
    def get(self) :
        ...
        try :
            ...
            query = '''select p.id as posting_id, p.img_url, p.content, 
                        u.username, p.created_at,
                        count(l.user_id) as like_cut
                        from posting p
                        join user u
                        on p.user_id = u.id
                        left join likes l
                        on l.posting_id = p.id
                        group by p.id
...
#비유저 포스팅 업로드
class nuPostingResource(Resource) :
    def post(self) :
